# BillGen Professional Audit — audit-001

**Overall risk: RED**

- Target: `BillGen BETA` at `baa42151324f983c5e4174b8f2ee6d3398500b00` (working tree dirty)
- Branch: `main`
- Run: 2026-08-28T10:05:21+00:00 → 2026-08-28T10:06:44+00:00
- Findings: 21
...
**Evidence.** No registered route matched the export pattern across 70 endpoints.
...
**Evidence.** No registered route matched the deletion pattern across 70 endpoints.
...
#### `ARCH-DOC-001` 8 endpoint(s) documented in system-architecture.html do not exist in the router tree

- **Severity** MEDIUM · **Confidence** 0.75 · **Type** recommendation · **Agent** architecture
- **Location** `docs/ARCHITECTURE/system-architecture.html`

**Evidence.** The document names these, and no registered route matches after parameter normalisation: DELETE /auth/sessions, GET /reports/export, POST /auth/password, POST /email/test, POST /invoices/{}/send, POST /privacy/delete-account, POST /privacy/export, POST /quotes/{}/convert-to-invoice

**Impact.** The architecture document is the stated source of truth for planning. Endpoints it describes that nobody can call turn into roadmap items believed to be done, and into frontend work written against a contract that was never shipped.
...
#### `ARCH-DOC-002` 50 registered endpoint(s) appear nowhere in system-architecture.html

- **Severity** LOW · **Confidence** 0.80 · **Type** recommendation · **Agent** architecture
- **Location** `docs/ARCHITECTURE/system-architecture.html`

**Evidence.** Registered and undocumented: DELETE /invoices/{}, DELETE /quotes/{}, GET /backup/export, GET /clients/{}, GET /companies, GET /credit-notes, GET /credit-notes/{}, GET /credit-notes/{}/html, GET /credit-notes/{}/pdf, GET /healthz, GET /invoices/{}, GET /invoices/{}/html, GET /invoices/{}/pdf, GET /orgs/current, GET /products/{}, GET /quotes, GET /quotes/{}, GET /readyz, GET /reports/invoices, GET /reports/kpi...

**Impact.** The document's endpoint inventory is presented as complete. Where it is not, a reader cannot tell absence-from-the-doc from absence-from-the-product, which is the property that made it worth maintaining.

**Recommendation.** Generate the inventory section from the router tree instead of maintaining it by hand. This scanner already produces the list; the document can consume it.

**Verify.** Re-run; present_but_undocumented should shrink.
...
**Evidence.** Code matching this rule's signal exists in 226 place(s), e.g. api/entitlements/service.py, api/routers/backup.py, api/routers/invoices.py, api/routers/quotes.py. A pattern match shows the topic is addressed somewhere; it does not show the rule is met. Only a reading of the implementation against the source can conclude that.
...
**P2 — premium / scale improvements** (12)
...
- `ARCH-DOC-001` 8 endpoint(s) documented in system-architecture.html do not exist in the router tree — MEDIUM, confidence 0.75
- `ARCH-DOC-002` 50 registered endpoint(s) appear nowhere in system-architecture.html — LOW, confidence 0.80
