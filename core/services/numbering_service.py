...
def allocate_quote_numbers(
    uow: UnitOfWork, company: Company, issue_date: date
) -> tuple[str, int]:
    """Returns (reference, sequence_global) for a new quote.

    Its own series, never the invoice one. There is no gapless obligation here —
    a quote nobody accepts is allowed to leave a hole — so the number is
    allocated at creation rather than withheld until some later act.
    """
    seq_global = uow.sequences.next_value(company.id, QUOTE_SERIES)
    reference = format_quote_reference(
        prefix=company.invoice_reference_prefix,
        year=issue_date.year,
        seq_global=seq_global,
    )
