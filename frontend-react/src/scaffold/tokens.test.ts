...
test("no raw hex in component or panel code", () => {
  ...
  //
  // `types/api.d.ts` is generated by `npm run generate:api` from the backend's
  // OpenAPI schema, so it carries whatever defaults the server declares —
  // including the designer's default brand colour. Hand-editing it would be
  // undone by the next generation, and a guard on hand-written colour has
  // nothing to say about a file no hand writes.
  const EXEMPT =
    /(?:[\\/]icons[\\/]|[\\/]scaffold[\\/]|[\\/]types[\\/]api\.d\.ts$|[\\/]workspace[\\/](?:templateSchema|templatePresets)\.ts$)/;
