...
| Tests | **Python 298 passed, 0 skipped** (`python -m pytest tests`); **frontend 157 passed** (`npm run test --workspace @billgen/ui`) |
...
| PDF engine | **Headless Chromium via Playwright** (the only engine, cross-platform incl. Windows/desktop). Setup on a fresh box: `pip install playwright` then `python -m playwright install chromium`. Without any engine, `/pdf` returns a clean 503. Free-tier PDFs carry a subtle "Made with BillGen" footer + logo. |
...
| `core/trust/` | `legal` (7 documents, none drafted), `consent` (4 cookie categories, `normalise()`), `personal_data` (GDPR art. 30 register + subprocessors), `security_events` (`SECURITY_ACTIONS`, `classify()`, `unrecorded()`), `ai_transparency` (`TRANSPARENCY_OBLIGATION_DATE`, Annex III watchlist) | **L4 registries — data, no I/O, no database.** Statements about the product that a test can keep true: what legal paperwork exists, what personal data is held and for how long, which cookies run, which audit actions are security events, which outputs a machine produced. Served publicly at `/trust/*` so the marketing site and the app read one source. Each module names its own gap (consent has no storage, the documents have no text) rather than hiding it. |
...
- Smaller: **DONE** — HistoryPanel now has **PDF + Peppol XML download buttons**
  ...
  Peppol 200, PDF 503 handled gracefully where no browser is installed). Still open:
