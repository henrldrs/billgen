...
| 04 | Code Quality | 8% | 3 | 4 | ▲ | **206 Python + 38 FE tests** (audit found *zero*); strict typing; layered |
...
| 12 | **No tests at all** — `lint` is `tsc --noEmit` | High | **✔ Solved** | **206 Python (0 skipped) + 38 frontend** tests. Tenant isolation, numbering, VAT math, Peppol all covered. |
| 13 | Peppol UBL 2.1 / EN 16931 **structurally done** (positive) | Med+ | **✔ Improved** | Kept, and extended: per-line VAT categories, **multi-rate** `TaxSubtotal`, discounts, structured comm, pre-export validation gate. Since then BE output was switched from UBL.BE to **plain BIS 3.0** and both BE and non-BE samples **Helger-validate clean** (OpenPeppol 2026.5) — see HANDOFF §9. |
...
- **UX polish** — the demo's Tailwind-v4 + Motion + glassmorphism look scored **UX 4**;
  the Beta shells are functional but plainer (est. **3**). *Update 2026-07: a
  from-scratch premium component library (glass, 3D buttons, dark mode, P0+P1
  complete) now exists at `docs/frontent build/component-library/` — but it is
  deliberately not wired into the live shells yet, so the shipped UX score doesn't
  move until integration.*
- **Access-Point validation of the XML** — still pending. *Update: the schematron
  half is closed — both BE and non-BE samples Helger-validate clean against
  OpenPeppol 2026.5 (see HANDOFF §9); what remains is a real Access-Point
  round-trip, i.e. transport, not format.*
