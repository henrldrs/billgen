...
| # | Item | Tier | BillGen | Evidence |
|---|---|---|---|---|
| **C1** | Database and storage | T0+ | ⚠️ | Postgres (prod) / SQLite (workstation), repository ports. **No blob storage — blocker B2.** Documents, company logos and expense scans all dangle on it. |
...
| **C3** | Backup, restore and retention | T1+ | ⚠️ | A real backup export/restore with an audit entry on both (`core/services/backup_service.py`, ADR-0003). **No automated backups, no restore ever rehearsed, and no retention policy expressed anywhere in code** — despite the seven-year Belgian obligation being the single hardest constraint on this product. An untested restore is a belief. |
